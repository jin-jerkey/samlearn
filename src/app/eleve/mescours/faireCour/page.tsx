...
export default function FaireCour() {
  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      ...
      try {
        ...
        const courseResponse = await fetch(`http://localhost:5000/api/cours/${coursId}/details?eleve_id=${eleveData.id}`);
  ...
  useEffect(() => {
    const fetchModuleProgress = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:5000/api/module/progress/${coursId}/${eleveData.id}`);
  ...
  useEffect(() => {
    const loadComments = async () => {
      if (!coursId) return;
      try {
        const response = await fetch(`http://localhost:5000/api/commentaires/cours/${coursId}`);
        const data = await response.json();
        if (data.status === 'success') {
  ...
  const handleModuleComplete = async (moduleId: number) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/module/complete', {
        method: 'POST',
        headers: {
  ...
  const handleCommentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/commentaire/create', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (data.status === 'success') {
        ...
        const commentsResponse = await fetch(`http://localhost:5000/api/commentaires/cours/${coursId}`);
          ...
          <div className="h-[400px] w-full">
            ...
            >
              <p>Le PDF ne peut pas être affiché. 
                <a href={`http://localhost:5000/${module.contenu}`} 
                   target="_blank"
      ...
      case 'vidéo':
        return (
          <video controls className="w-full max-h-[400px]">
            <source src={`http://localhost:5000/${module.contenu}`} type="video/mp4" />
        ...
        <main className="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 p-6">
          {courseData && (
            <div className="container mx-auto">
              <h1 className="text-2xl font-bold text-gray-900 mb-6">{courseData.titre}</h1>
                ...
                <div className="w-[70%] space-y-6">
                  ...
                  <div className="flex justify-between">
                    ...
                    {currentModuleIndex === modules.length - 1 ? (
                      <button
                        onClick={() => router.push(`/eleve/mescours/fairExamin?cours_id=${coursId}`)}
